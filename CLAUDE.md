...
設計レビューが完了したら、**実装に入る前に**必ず以下を実行すること：

```
1. tasks/<id>/LOG.md を作成
2. 設計レビュー結果をLOG.mdに記録
3. その後、実装を開始
```

**理由:**
- クラッシュ時に設計レビュー結果が消失する
- 再度設計レビューを実行するとトークンが無駄になる
- 実装途中でクラッシュしても、設計判断は残る

**LOG.mdに記録する内容:**
- 設計レビューの評価（承認/Blocker付き承認/要修正）
- Decision Required項目の選択結果
- Info項目（実装時の注意事項）
- 作成予定ファイル一覧
