...
タスク実行中は**各フェーズ完了ごとに即時記録**すること。
「全部終わってから記録」は禁止。クラッシュで全て消失する。

**記録タイミングと内容:**

| フェーズ | 記録タイミング | 記録内容 |
|----------|---------------|----------|
| タスク開始 | `/task`実行直後 | LOG.md作成、タスク概要 |
| 設計レビュー | レビュー完了直後 | 評価、Decision項目の選択、Info項目 |
| 実装 | 各機能/ファイル完了ごと | 作成ファイル、設計判断、注意点 |
| テスト | テスト作成/実行後 | テスト結果、カバレッジ |
| コードレビュー | レビュー完了直後 | 指摘事項、対応方針 |
| 修正 | 各修正完了ごと | 修正内容、コミットハッシュ |

**なぜ細かく記録するのか:**
- クラッシュは予測不能。いつ落ちても復旧できる状態を維持する
- 設計レビュー、調査結果はトークンを大量消費する。再実行は無駄
- 「あとで記録」は「記録しない」と同義
...
LOG.md作成 ← ★ここで作成
  ↓
設計レビュー → LOG.mdに追記 ← ★即時記録
  ↓
実装（機能A）→ LOG.mdに追記 ← ★即時記録
  ↓
実装（機能B）→ LOG.mdに追記 ← ★即時記録
  ↓
テスト作成 → LOG.mdに追記 ← ★即時記録
  ↓
コードレビュー → LOG.mdに追記 ← ★即時記録
  ↓
コミット・PR
```
...
タスク実行中にユーザーから追加要望・指摘・判断があった場合も即時記録する。

| 対話種別 | 記録タイミング | 記録内容 |
|----------|---------------|----------|
| 追加要望 | 受領直後 | 要望内容、対応方針 |
| 指摘・修正依頼 | 受領直後 | 指摘内容、原因、修正方針 |
| 判断・承認 | 決定直後 | 判断内容、選択肢、理由 |
| 質問への回答 | 回答直後 | 質問、回答、影響範囲 |
...
#### 追加要望: 段階的記録ルールの強化
- **要望:** タスク基本フローだけでなく、追加要望も随時記録すること
- **対応:** CLAUDE.mdにユーザー対話の記録ルールを追加
```
...
| 判断 | 条件 |
|------|------|
| 今回対応すべき | 軽微な修正（30分以内）、または未対応だと問題が拡大する場合 |
| 次回対応でOK | 設計変更を伴う、または他タスクへの影響が大きい場合 |
| タスク化不要 | 既存の仕組みで解決できる、または問題の前提が誤っている場合 |

**⚠️ 重要: 軽微な修正は即時対応**

以下は「次回対応」にせず、**今回のタスク内で対応する**:
- インターフェース/型へのJSDoc追加（数行）
- 定数化（数行〜数十行）
- フォーマット修正
- typo修正
- コメント追加
...
**具体例:**
- **今回対応すべき**: JSDoc追加、定数化、Docstring追加、セキュリティ脆弱性、テスト失敗
- **次回対応でOK**: アーキテクチャ変更、複数機能にまたがるリファクタリング
- **タスク化不要**: 既存importで解決できる問題、前提が誤っている指摘
