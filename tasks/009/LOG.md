...
## ワークフローレビュー（2026-02-09）

### 問題の根本原因分析

| 問題 | 直接原因 | 根本原因 |
|------|----------|----------|
| Blackフォーマット未適用 | コミット前にBlackを実行しなかった | pre-commitがホスト環境にインストールされていない |
| flake8エラー | コミット前にflake8を実行しなかった | 同上 |
| CI PostGIS接続エラー | test.pyがDATABASE_URL未対応 | CIとローカルの環境差異を事前検証していなかった |
| マイグレーション未確認 | `python manage.py migrate`を実行しなかった | CLAUDE.md「13. コミット前チェック」に手順が含まれていなかった |
