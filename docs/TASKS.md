...
## Claude Code Skills [8/8]

スラッシュコマンドで呼び出せるスキルの実装。
既存スクリプトの機能をスキルに移行し、開発効率を向上させる。
...
- **ブランチ:** feature/skills
- **完了日:** 2025-01-29
- **成果物:** `.claude/commands/spec.md`
- **機能:** SPEC.mdの特定セクションを参照
- **使用例:** `/spec 4.3.1` → Location APIの仕様表示
- **チェックリスト:**
  - [x] スキルファイル作成
  - [x] セクション番号でのフィルタリング
  - [x] 動作確認
...
- **ブランチ:** feature/skills
- **完了日:** 2025-01-29
- **成果物:** `.claude/commands/guide.md`
- **機能:** CLAUDE.mdのコーディング規約を参照
- **使用例:** `/guide constants` → 定数管理ルール表示
- **チェックリスト:**
  - [x] スキルファイル作成
  - [x] キーワードでのフィルタリング
  - [x] 動作確認
...
- **ブランチ:** feature/skills
- **完了日:** 2025-01-29
- **成果物:** `.claude/commands/progress.md`
- **機能:** TASKS.mdから進捗サマリーを表示
- **使用例:** `/progress` → 全体進捗とフェーズ別進捗表示
- **チェックリスト:**
  - [x] スキルファイル作成
  - [x] 進捗計算ロジック
  - [x] 動作確認
...
- **ブランチ:** feature/skills
- **完了日:** 2025-01-29
- **成果物:** `.claude/commands/setup.md`
- **機能:** 環境構築ガイドを表示
- **使用例:** `/setup local` → LOCAL_SETUP.md表示
- **チェックリスト:**
  - [x] スキルファイル作成
  - [x] 環境種別でのドキュメント切り替え
  - [x] 動作確認

### ✅ #D022 /checklist スキル作成
- **優先度:** 中
- **見積:** 0.5h
- **依存:** #D018
- **ブランチ:** feature/skills
- **完了日:** 2025-01-29
- **成果物:** `.claude/commands/checklist.md`
- **機能:** タスクのチェックリストを表示・更新
- **使用例:** `/checklist D007` → チェックリスト表示
- **チェックリスト:**
  - [x] スキルファイル作成
  - [x] チェックリスト抽出ロジック
  - [x] 動作確認

### ✅ #D023 /review スキル作成（スクリプト移行）
- **優先度:** 中
- **見積:** 0.5h
- **依存:** #D018
- **ブランチ:** feature/skills
- **完了日:** 2025-01-29
- **成果物:** `.claude/commands/review.md`
- **機能:** 変更内容のレビュー（review_changes.shの移行）
- **使用例:** `/review` → 現在の変更をレビュー形式で表示
- **移行元:** `scripts/claude/review_changes.sh`
- **チェックリスト:**
  - [x] スキルファイル作成
  - [x] git status/diff統合
  - [x] SPEC/CLAUDE準拠チェック
...
### ✅ #D024 /pr スキル作成（スクリプト移行）
- **優先度:** 中
- **見積:** 0.5h
- **依存:** #D018
- **ブランチ:** feature/skills
- **完了日:** 2025-01-29
- **成果物:** `.claude/commands/pr.md`
- **機能:** Draft PR作成補助（create_draft_pr.shの移行）
- **使用例:** `/pr D007` → PR作成
- **移行元:** `scripts/claude/create_draft_pr.sh`
- **チェックリスト:**
  - [x] スキルファイル作成
  - [x] PR本文テンプレート生成
  - [x] gh pr create統合
...
- **成果物:**
  - `CLAUDE.md`（更新）
  - `docs/SKILL.md`（新規）
- **機能:** タスク実行時の頻出処理をスキル化するかの確認フロー
- **ルール:**
  - 頻出処理を検出したら「スキルに追加しますか？(y/N)」と確認
  - yで追加、重複チェックあり
- **チェックリスト:**
  - [x] CLAUDE.mdにスキル追加ルール記載
  - [x] SKILL.md作成（スキル管理ドキュメント）
  - [x] 重複チェックロジック
  - [x] 動作確認
...
### ⬜ #TECH-001 認証API定数化・Docstring完全化
- **優先度:** 低
- **見積:** 1h
- **発生元:** #D006（code-reviewerレビュー）
- **ブランチ:** refactor/auth-constants
- **内容:**
  - JWT設定のマジックナンバー定数化（base.py:206-207）
  - テストパスワードの定数化（conftest.py）
  - display_name max_length定数化（serializers.py:23）
  - DocstringへのExampleセクション追加（serializers.py複数箇所）
- **チェックリスト:**
  - [ ] JWT_ACCESS_TOKEN_LIFETIME定数化
  - [ ] JWT_REFRESH_TOKEN_LIFETIME定数化
  - [ ] TEST_PASSWORD定数化
  - [ ] DISPLAY_NAME_MAX_LENGTH定数化
  - [ ] Docstring Example追加

### ⬜ #TECH-002 OAuthトークン暗号化
- **優先度:** 低 → **個人利用を超える場合は最高（必須）**
- **見積:** 2h
- **発生元:** #007（実装前設計レビュー）
- **ブランチ:** security/oauth-token-encryption
- **トリガー条件:** 個人利用から複数ユーザー利用に移行する時点で**必須対応**
- **SPEC参照:** SPEC.md § 8.2「OAuthトークン: 暗号化して保存」
- **内容:**
  - allauthのSocialTokenモデルのトークンを暗号化
  - django-fernet-fields または django-encrypted-model-fields の導入
  - 既存トークンのマイグレーション
- **チェックリスト:**
  - [ ] 暗号化ライブラリ選定
  - [ ] SocialTokenモデルのカスタマイズ
  - [ ] 既存トークンの暗号化マイグレーション
  - [ ] 暗号化キーの環境変数管理
  - [ ] テスト作成

### ⬜ #TECH-003 SPEC/CLAUDE整合性確保
- **優先度:** 中
- **見積:** 2h
- **発生元:** #D018（テスト戦略調査）
- **ブランチ:** docs/spec-claude-alignment
- **内容:**
  - SPEC.md § 9.1.1 カバレッジ目標の根拠追記
  - SPEC/CLAUDE間のカバレッジ目標不一致解消
  - CLAUDE.mdからSPEC.mdへの参照形式統一
- **チェックリスト:**
  - [ ] SPEC.md § 9.1.1 根拠セクション追加
  - [ ] CLAUDE.md カバレッジ目標をSPEC.md参照形式に統一
  - [ ] 不一致項目（Models/Components/Utils）の解消

### ⬜ #TECH-004 テストケース分類ガイドライン
- **優先度:** 中
- **見積:** 2h
- **発生元:** #D018（テスト戦略調査）
- **ブランチ:** docs/test-classification
- **内容:**
  - SPEC.md § 9.5 テストケース分類セクション追加
  - SPEC.md § 9.6 境界値テスト例の追加
  - qa-test-engineer向けレビュー指針
- **チェックリスト:**
  - [ ] SPEC.md § 9.5 テストケース分類追加
  - [ ] SPEC.md § 9.6 境界値テスト例追加
  - [ ] 境界値テストの必須対象リスト作成
  - [ ] テスト命名規則の明文化

### ⬜ #TECH-005 E2E/セキュリティテスト方針策定
- **優先度:** 低
- **見積:** 3h
- **発生元:** #D018（テスト戦略調査）
- **トリガー条件:** MVP完成後
- **ブランチ:** docs/e2e-security-testing
- **内容:**
  - E2Eテスト戦略（Playwright/Cypress選定）
  - セキュリティテスト方針
  - パフォーマンステスト方針
- **チェックリスト:**
  - [ ] E2Eテストフレームワーク選定
...
- **チェックリスト:**
  - [x] django-filter設定
  - [x] LocationFilterSet作成
  - [x] テキスト検索実装
  - [x] カテゴリフィルタ
  - [x] タグフィルタ（OR条件、SPEC未定義のためUX慣行採用）
  - [x] ステータスフィルタ
  - [x] ソート機能（visited_at追加）
  - [x] テスト作成（23件）
- **成果物:**
