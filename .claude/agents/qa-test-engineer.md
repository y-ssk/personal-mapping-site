...
- **リグレッションテスト（Regression tests）** - 既存機能への影響確認
- **デグレードテスト（Degradation tests）** - 性能・品質の劣化確認

### リグレッション/デグレード観点（必須）

変更を加える際、以下の観点で既存機能への影響を検証する:

| 観点 | 確認内容 | 確認方法 |
|------|---------|---------|
| 依存関係 | 変更対象に依存するコードへの影響 | `findReferences`、importの追跡 |
| データ整合性 | 既存データとの互換性 | マイグレーション検証、既存データでのテスト |
| API互換性 | エンドポイントのリクエスト/レスポンス形式 | OpenAPI diff、既存クライアントでの動作確認 |
| 暫定実装 | 暫定対応が他機能に与える影響 | 将来タスクとの依存関係確認 |

**暫定実装に関する特別ルール:**

暫定実装（Value(0)等のプレースホルダー）が存在する場合:
1. 暫定実装の影響範囲を明記する
2. 本実装時の変更箇所を特定する
3. 破壊的変更の有無を判定する
4. 破壊的変更がある場合は**Blocker**として報告する
